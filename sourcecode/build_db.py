...
def create_credibility_knowledge_table():
    db = MySQLdb.connect(host=constants.HOST, user=constants.USER, passwd=constants.PWD, db=constants.DB,
                         port=constants.PORT, charset='utf8')
    cursor = db.cursor()
...
def init(host, user, pw, dbname):
    ...
    fuzzy_insert_all(host, user, pw, dbname)
    cnt.close()


def init_credibility():
    create_credibility_knowledge_table()
    add_credibility_knowledge("0 to 5 cars passed in 15 seconds", "Green light on for 8 second", 0.8, 0.55, 'Jz')
    add_credibility_knowledge("5 to 10 cars passed in 15 seconds", "Green light on for 16 second", 0.8, 0.55, 'Jz')
    add_credibility_knowledge("10 to 15 cars passed in 15 seconds", "Green light on for 24 second", 0.8, 0.55, 'Jz')
    add_credibility_knowledge("More than 15 cars passed in 15 seconds", "Green light on for 30 second", 0.8, 0.55, 'Jz')
    credibility.create_truth_table()
    credibility.add_truth("0 to 5 cars passed in 15 seconds", 0.8, "Jz")
    credibility.add_truth("5 to 10 cars passed in 15 seconds", 0.8, "Jz")
    credibility.add_truth("10 to 15 cars passed in 15 seconds", 0.8, "Jz")
    credibility.add_truth("More than 15 cars passed in 15 seconds", 0.8, "Jz")
    statistic_table.create_statistic_table()
...
def fuzzy_knowledge_change(conn, dbname, id, var1_list, cpt1_func, var2_list, cpt2_func, var1, var2, cpt1=None,
    ...
    knowledge = list(fuzzy_knowledge_show(conn, dbname, id))
